#!/bin/bash
# NitroGen — future_action_direct 推理服务启动脚本
# 模型: NitroGen_cuphead_future_action_director_F18_2head
# action_mapping: gamepad_direct (2-head: button BCE + stick CE，无扳机)
...
# 配置变量（按需修改）
# ============================================================
CONFIG_FILE="config/policy_model/150M_local_nitrogen_dataset_future_action_direct_inference.yaml"
CHECKPOINT_PATH=""   # 必填：请指定 checkpoint 路径，例如 output_20260415/checkpoint-step=00XXXXXX.ckpt
...
# ============================================================
usage() {
    echo "用法: bash scripts/inference_future_action_direct.sh [选项]"
    echo
    echo "选项:"
...
# ============================================================
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}NitroGen future_action_direct 推理服务${NC}"
echo -e "${GREEN}========================================${NC}"
...
if [ -z "$CHECKPOINT_PATH" ]; then
    echo -e "${RED}错误: 请通过 -k 指定 checkpoint 路径${NC}"
    exit 1
fi
if [ ! -e "$CHECKPOINT_PATH" ]; then
    echo -e "${RED}错误: checkpoint 不存在: $CHECKPOINT_PATH${NC}"
    echo -e "${YELLOW}提示: 当前工作目录: $(pwd)${NC}"
    exit 1
fi
...
echo -e "${CYAN}  action_mapping: gamepad_direct (2-head)${NC}"
