#!/bin/bash
# 使用本地 dataset 目录验证 baseline（无 s0）Stage3 模型脚本
...
CHECKPOINT_PATH="output/policy_model/150M_nitrogen/stage3_finetune/checkpoint-step=00030000.ckpt"
TEMP_CONFIG_FILE=""
MIN_STEPS=""
MAX_STEPS=""
...
usage() {
    echo "用法: bash scripts/validate_local_dataset.sh [-d 数据集路径] [-c 配置文件] [-k checkpoint路径] [--min_steps N] [--max_steps N]"
    echo
    echo "参数:"
    echo "  -d    数据集路径 (默认: NitroGen_cuphead_toy)"
    echo "  -c    配置文件路径 (默认: config/policy_model/150M_local_nitrogen_dataset_future.yaml)"
    echo "  -k    checkpoint 路径 (默认: output/policy_model/150M_nitrogen/stage3_finetune/checkpoint-step=00030000.ckpt)"
    echo "  --min_steps   最小 step（可选，仅当 checkpoint_dir 是目录时有意义）"
    echo "  --max_steps   最大 step（可选，仅当 checkpoint_dir 是目录时有意义）"
    echo "  -h    显示帮助"
    echo
    echo "示例:"
    echo "  bash scripts/validate_local_dataset.sh"
    echo "  bash scripts/validate_local_dataset.sh -d NitroGen_cuphead_toy"
    echo "  bash scripts/validate_local_dataset.sh -d NitroGen_cuphead_toy -k output/policy_model/150M_nitrogen/stage3_finetune/checkpoint-step=00030000.ckpt"
}
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Open P2P Baseline 验证脚本${NC}"
echo -e "${GREEN}========================================${NC}"
