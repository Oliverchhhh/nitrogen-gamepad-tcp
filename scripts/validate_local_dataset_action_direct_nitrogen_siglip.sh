#!/bin/bash
# Teacher-forcing 验证脚本（nitrogen_siglip direct action）
...
LOG_FILE="${LOG_DIR}/validate_action_direct_nitrogen_siglip_${TIMESTAMP}.log"
...
# CHECKPOINT_PATH="output/policy_model/150M_nitrogen_cuphead_future_action_direct_siglip_F5_test"
CHECKPOINT_PATH="output/policy_model/150M_nitrogen_cuphead_future_action_direct_siglip_F5_onelevel1/stage3_finetune/checkpoint-step=00015000.ckpt"
TEMP_CONFIG_FILE=""
MIN_STEPS=""
MAX_STEPS=""
N_SEQUENCES="225"
...
usage() {
    echo "用法: bash scripts/validate_local_dataset_action_direct_nitrogen_siglip.sh [-d 数据集] [-c 配置] [-k checkpoint路径/目录] [-g GPU_ID] [--no_wandb] [--min_steps N] [--max_steps N] [--frame_index N] [--all_future_frames]"
    echo
    echo "参数:"
    echo "  -d    验证数据集路径 (默认: cuphead_dataset_converted/v350335326)"
    echo "  -c    配置文件路径 (默认: config/policy_model/150M_local_nitrogen_dataset_future_action_direct_nitrogen_siglip.yaml)"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Nitrogen SigLIP Direct Action 验证脚本${NC}"
echo -e "${GREEN}========================================${NC}"
...
TEMP_CONFIG_FILE=$(mktemp /tmp/open_p2p_validate_direct_siglip_config.XXXXXX.yaml)
...
export TORCHINDUCTOR_CACHE_DIR=".elefant_temp_validation_direct_siglip/torch_compiler/inductor_cache"
