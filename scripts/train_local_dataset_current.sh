#!/bin/bash
# 使用本地 dataset 目录训练带 Current Vision 目标的 Stage3 模型的脚本
...
usage() {
    echo "用法: bash scripts/train_local_dataset_current.sh [-d 数据集路径] [-o 输出目录] [-c 配置文件] [-n]"
    echo
    echo "参数:"
    ...
    echo
    echo "示例:"
    echo "  bash scripts/train_local_dataset_current.sh"
    echo "  bash scripts/train_local_dataset_current.sh -c config/policy_model/150M_local_nitrogen_dataset_current.yaml -d NitroGen_cuphead"
    echo "  bash scripts/train_local_dataset_current.sh -c config/policy_model/150M_local_nitrogen_dataset_current.yaml -d NitroGen_cuphead -n"
}
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Open P2P 本地数据集 Current Vision 训练脚本${NC}"
echo -e "${GREEN}========================================${NC}"
...
if [ -n "$OUTPUT_DIR" ]; then
    ...
    mkdir -p "$OUTPUT_DIR"
    TEMP_CONFIG_FILE=$(mktemp /tmp/open_p2p_current_train_config.XXXXXX.yaml)
...
export TORCHINDUCTOR_CACHE_DIR=".elefant_temp_current/torch_compiler/inductor_cache"
...
# 5. 开始训练
echo -e "\n${YELLOW}[5/5] 开始训练 (Current Vision)...${NC}"
...
echo -e "\n${GREEN}========================================${NC}"
echo -e "${GREEN}Current Vision 训练完成！${NC}"
echo -e "${GREEN}========================================${NC}"
