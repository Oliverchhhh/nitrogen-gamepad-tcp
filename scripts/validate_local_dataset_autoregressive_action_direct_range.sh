#!/bin/bash
# 对 150M_nitrogen_cuphead_future_action_direct_F18_2head_all 指定 step 范围跑自回归验证
# 范围: step=10000 ~ step=180000
# 已有 validation_ar_direct_step*.json 的 checkpoint 自动跳过

set -e

CKPT_DIR="output/policy_model/150M_nitrogen_cuphead_future_action_direct_F18_2head/stage3_finetune"
...
echo -e "${GREEN}========================================"
echo -e "150M_nitrogen_cuphead_future_action_direct_F18_2head_all 自回归批量验证"
echo -e "step 范围: ${STEP_MIN} ~ ${STEP_MAX}"
echo -e "========================================${NC}"
echo "checkpoint 目录: $CKPT_DIR"
echo "数据集: $DATA_FOLDER"
echo "序列数: $N_SEQUENCES"
echo ""
...
for ckpt in "${CKPTS[@]}"; do
    ...
    ar_json="$CKPT_DIR/validation_ar_direct_step${step}.json"
    ...
    bash scripts/validate_local_dataset_autoregressive_action_direct.sh \
        -k "$ckpt" \
        -d "$DATA_FOLDER" \
        -c "$CONFIG_FILE" \
        -n "$N_SEQUENCES" \
