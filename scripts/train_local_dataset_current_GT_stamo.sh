#!/bin/bash
# 使用本地 dataset 目录训练 Current Vision（输入 conv，state GT 使用 StaMo）的 Stage3 模型脚本

set -e  # 遇到错误立即退出

# 配置变量
CONFIG_FILE="config/policy_model/150M_local_nitrogen_dataset_current_GT_stamo.yaml"
...
usage() {
    echo "用法: bash scripts/train_local_dataset_current_GT_stamo.sh [-d 数据集路径] [-o 输出目录] [-c 配置文件] [-n]"
    echo
    echo "参数:"
    echo "  -d    数据集路径 (默认: dataset)"
    echo "  -o    输出目录。传入后将覆盖配置文件中的 shared.output_path"
    echo "  -c    配置文件路径 (默认: config/policy_model/150M_local_nitrogen_dataset_current_GT_stamo.yaml)"
    echo "  -n    禁用 torch.compile（向 train_future.py 透传 --no_compile）"
    echo "  -h    显示帮助"
    echo
    echo "示例:"
    echo "  bash scripts/train_local_dataset_current_GT_stamo.sh"
    echo "  bash scripts/train_local_dataset_current_GT_stamo.sh -c config/policy_model/150M_local_nitrogen_dataset_current_GT_stamo.yaml -d NitroGen_cuphead"
    echo "  bash scripts/train_local_dataset_current_GT_stamo.sh -c config/policy_model/150M_local_nitrogen_dataset_current_GT_stamo.yaml -d NitroGen_cuphead -n"
}
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Open P2P Current 训练脚本（GT: StaMo）${NC}"
echo -e "${GREEN}========================================${NC}"
...
if [ -n "$OUTPUT_DIR" ]; then
    ...
    mkdir -p "$OUTPUT_DIR"
    TEMP_CONFIG_FILE=$(mktemp /tmp/open_p2p_current_gt_stamo_train_config.XXXXXX.yaml)
...
export TORCHINDUCTOR_CACHE_DIR=".elefant_temp_current_gt_stamo/torch_compiler/inductor_cache"
...
# 5. 开始训练
echo -e "\n${YELLOW}[5/5] 开始训练 (Current + GT StaMo)...${NC}"
...
echo -e "\n${GREEN}========================================${NC}"
echo -e "${GREEN}Current + GT StaMo 训练完成！${NC}"
echo -e "${GREEN}========================================${NC}"
