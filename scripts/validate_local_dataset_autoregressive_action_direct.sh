...
CHECKPOINT_PATH="output/policy_model/150M_nitrogen_cuphead_future_action_direct_F18_2head_all/stage3_finetune"
N_SEQUENCES=72
GPU_ID="0"
...
usage() {
    echo "用法: bash scripts/validate_local_dataset_autoregressive_action_direct.sh -k checkpoint路径 [-d 数据集] [-c 配置] [-n 序列数] [-g GPU_ID]"
    echo
    echo "参数:"
...
while getopts ":k:d:c:n:g:h" opt; do
    case "$opt" in
