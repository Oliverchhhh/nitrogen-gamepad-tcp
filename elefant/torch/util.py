...
# 使用项目目录下的临时目录，避免占用C盘空间
# 优先使用环境变量，如果没有则使用项目目录
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_TEMP_BASE = os.environ.get("ELEFANT_TEMP_DIR", os.path.join(_PROJECT_ROOT, ".elefant_temp"))
ELEFANT_WANDB_DIR = os.path.join(_TEMP_BASE, "wandb")
ELEFANT_ZMQ_DIR = os.path.join(_TEMP_BASE, "zmq")
...
def pytorch_setup(set_seed: bool = False):
    ...
    # Keep wandb logs out of NFS and C drive.
    os.makedirs(ELEFANT_WANDB_DIR, exist_ok=True)
    os.environ["WANDB_DIR"] = ELEFANT_WANDB_DIR
    # ZMQ临时目录
    os.makedirs(ELEFANT_ZMQ_DIR, exist_ok=True)
    ...
    torch_cache_dir = os.path.join(_TEMP_BASE, "torch_compiler", "inductor_cache")
