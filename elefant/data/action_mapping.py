...
class GamepadAutoregressiveActionMappingConfig(ConfigBase):
    """
    Configuration for gamepad autoregressive action mapping.

    This maps GamePadAction (NitroGen-style controller space) into a fixed-length
    sequence of discrete tokens that can be used by the policy model, similar to
    UniversalAutoregressiveActionMapping for keyboard+mouse.
    """

    # Maximum number of buttons that can be represented at once.
    max_buttons: int = pydantic.Field(
        default=6,
        ge=1,
        le=16,
        description="Maximum number of gamepad buttons that can be represented per frame",
    ) #每帧最多保留6个同时安下的手柄按钮
    ...
    n_trigger_bins: int = pydantic.Field(
        default=17,
        ge=3,
        description="Number of quantization bins for trigger values in [0, 1]",
    )#将手柄的trigger值量化为17个bins
...
# These names correspond to the GamePadButtons field names in video_annotation.proto,
# plus virtual entries for stick-press events (left_thumb/right_thumb).
GAMEPAD_BUTTON_ENCODING_MAP = {
    "_no_button": 0,
    "south": 1,
    "north": 2,
    "east": 3,
    ...
    "left_thumb": 13,
    "right_thumb": 14,
} #这个按钮编码映射表，将手柄的按钮映射为0-14的整数，不一定对，待确认

# Fixed order in which we consider buttons when constructing the per-frame set.
GAMEPAD_BUTTON_ORDER = [
    ...
    "left_thumb",
    "right_thumb",
] #这个按钮顺序表，对照在每帧中，哪些按钮会被考虑


class GamepadAutoregressiveActionMapping:
    """
    Map GamePadAction (gamepad controller space) to a fixed-length sequence of tokens.

    Each frame is represented as:
        [btn_1, ..., btn_K, lx, ly, rx, ry, lt, rt]

    - btn_i: indices into GAMEPAD_BUTTON_ENCODING_MAP (0 = _no_button)
    - lx, ly, rx, ry: quantized stick axes in [-1, 1] with n_stick_bins
    - lt, rt: quantized trigger values in [0, 1] with n_trigger_bins
    """

    def __init__(
        self, config: Optional[GamepadAutoregressiveActionMappingConfig] = None
    ):
        if config is None:
            config = GamepadAutoregressiveActionMappingConfig()
        self.config = config

    def get_seq_len(self) -> int:
        """Return the length of the autoregressive sequence for a single frame."""
    ...
    def make_empty_action(self, T: int) -> torch.Tensor:
        ...
        Shape: (T, seq_len), all zeros correspond to:
        - no buttons pressed
        - neutral sticks
        - zero triggers
        """
        device = torch.device("cpu")
        return torch.zeros(T, self.get_seq_len(), dtype=torch.long, device=device)
    ...
    def _quantize_trigger(self, value: float) -> int:
        """Quantize a trigger value in [0, 1] to [0, n_trigger_bins-1]."""
    ...
    def action_to_tensor(self, gamepad_actions) -> torch.Tensor:
        """
        Convert a GamePadAction proto message into a token tensor of shape (1, seq_len).

        The input is expected to be a `GamePadAction` from `video_annotation_pb2`,
        but we keep the type annotation loose here to avoid import cycles.
        """
        parsed = parse_gamepad_actions(gamepad_actions)

        # If there is no meaningful gamepad action, return a single no-op action.
        if parsed is None:
            return self.make_empty_action(T=1)

        pressed_buttons_proto = set(parsed["pressed_buttons"])
        ...
        # Build the logical set of pressed button names in our vocabulary.
        pressed_logical = set()

        # Map proto button field names directly where possible.
        for name in pressed_buttons_proto:
            if name in GAMEPAD_BUTTON_ENCODING_MAP:
                pressed_logical.add(name)

        # Incorporate stick-press events as separate logical buttons.
        if left_pressed:
            pressed_logical.add("left_thumb")
        if right_pressed:
            pressed_logical.add("right_thumb")

        # Order and truncate buttons according to GAMEPAD_BUTTON_ORDER.
        ordered_button_ids: List[int] = []
        for name in GAMEPAD_BUTTON_ORDER:
            if name in pressed_logical:
                ordered_button_ids.append(GAMEPAD_BUTTON_ENCODING_MAP[name])
            if len(ordered_button_ids) >= self.config.max_buttons:
                break

        # Pad with _no_button until we reach max_buttons.
        while len(ordered_button_ids) < self.config.max_buttons:
            ordered_button_ids.append(GAMEPAD_BUTTON_ENCODING_MAP["_no_button"])

        # Quantize sticks and triggers.
        lx_bin = self._quantize_stick(left_stick_x)
        ly_bin = self._quantize_stick(left_stick_y)
        rx_bin = self._quantize_stick(right_stick_x)
        ry_bin = self._quantize_stick(right_stick_y)
        lt_bin = self._quantize_trigger(left_trigger)
        rt_bin = self._quantize_trigger(right_trigger)

        tokens = ordered_button_ids + [lx_bin, ly_bin, rx_bin, ry_bin, lt_bin, rt_bin]
