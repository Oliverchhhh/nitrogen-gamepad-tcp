...
class Transformer(nn.Module):
    ...
    def construct_transformer_layers(self):
        ...
        assert embed_size_per_head % 2 == 0, "rotary embedding dim must be even" #rotary embedding dim must be even
        assert self.config.embed_dim % self.config.n_q_head == 0 #embed_dim must be divisible by n_q_head
        self.pos_embeddings = RotaryPositionalEmbeddings(
            dim=embed_size_per_head, max_seq_len=self.max_seq_len, base=10_000 #init pos_embeddings with ROPE
        ) #pos_embeddings: [max_seq_len, embed_size_per_head]

        transformer_layers = [] #list of transformer layers
        for i in range(self.config.n_transformer_layers): #for each transformer layer, 循环构建3层Transformer层
            sa = self.construct_self_attention() #construct self attention
            ffn = PackedSwiGLUFFN(
                dim=self.config.embed_dim,
                hidden_dim=self.config.embed_dim * 4,
                multiple_of=8,
            ) #ffn: [embed_dim, 4*embed_dim]
            layer = TransformerSelfAttentionLayer(
                dim=self.config.embed_dim,
                self_attention=sa,
                ffn=ffn,
            ) #layer: [embed_dim, embed_dim]
            transformer_layers.append(layer) #append layer to transformer layers

        self.transformer_layers = nn.ModuleList(transformer_layers) #transformer_layers: list of transformer layers
