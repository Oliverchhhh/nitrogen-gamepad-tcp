...
uv run elefant/policy_model/validation.py \
  ...
  --config_path=config/policy_model/<150M|300M|600M|1200M>.yaml
```

> **Important:** The config must match the checkpoint model size. Prefer the updated configs in `config/policy_model/` instead of relying on any legacy config bundled with checkpoints.
...
### Alternative Setup: Streaming with Moonlight
...
**Architecture:**
- **Game Machine (Laptop/PC)**: Runs the game and streams the display via Sunshine (Moonlight server)
- **Inference Machine (Windows PC)**: Runs Recap (captures Moonlight window) + WSL (model inference)

**Benefits:**
- ✅ Avoids resource competition between game rendering and model inference
- ✅ Each machine can focus on its primary task (rendering vs. inference)
- ✅ No need for dual GPU setup on a single machine
...
4. **In Recap**: Select the Moonlight window as the capture target instead of the game window directly

For detailed setup instructions, see [`MOONLIGHT_STREAMING_SETUP.md`](MOONLIGHT_STREAMING_SETUP.md).

**Note**: Streaming adds network latency (typically 10-50ms). Use wired network connection for best results.
